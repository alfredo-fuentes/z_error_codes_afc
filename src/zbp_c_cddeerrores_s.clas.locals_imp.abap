CLASS LHC_ZI_CDDEERRORES_S DEFINITION FINAL INHERITING FROM CL_ABAP_BEHAVIOR_HANDLER.
  PRIVATE SECTION.
    METHODS:
      AUGMENT_CDDEERRORES FOR MODIFY
        IMPORTING
          ENTITIES_CREATE FOR CREATE CDDeErroresAll\_CDDeErrores
          ENTITIES_UPDATE FOR UPDATE CDDeErrores.
ENDCLASS.

CLASS LHC_ZI_CDDEERRORES_S IMPLEMENTATION.
  METHOD AUGMENT_CDDEERRORES.
    DATA: text_for_new_entity      TYPE TABLE FOR CREATE ZI_CDDeErrores\_CDDeErroresText,
          text_for_existing_entity TYPE TABLE FOR CREATE ZI_CDDeErrores\_CDDeErroresText,
          text_update              TYPE TABLE FOR UPDATE ZI_CDDeErroresText.
    ...
    DATA: text_tky_link  TYPE STRUCTURE FOR READ LINK ZI_CDDeErrores\_CDDeErroresText,
    ...
    LOOP AT entities_create INTO DATA(entity).
      ...
      ENDLOOP.
    ENDLOOP.
    MODIFY AUGMENTING ENTITIES OF ZI_CDDeErrores_S
      ENTITY CDDeErrores
        CREATE BY \_CDDeErroresText
        FROM text_for_new_entity
        RELATING TO entities_create BY relates_create.

    IF entities_update IS NOT INITIAL.
      READ ENTITIES OF ZI_CDDeErrores_S
        ENTITY CDDeErrores BY \_CDDeErroresText
